...
public class GetNamesGeneEq extends ReadZipFile
{
	...
	protected boolean getNamesGenesFromFiles(String theInternalFile) throws IOException
	{
		...
		if(true==found)
		{
			TcgaGSData.printWithFlag("Gene list (" + mGenes.length + ") retrieved for " + theInternalFile + " in " + ((finish-start)/1000.0) + " seconds");
		}
		else
		{
			TcgaGSData.printWithFlag("Gene list not found for " + theInternalFile + " in " + ((finish-start)/1000.0) + " seconds");
		}
		return found;
	}
